window.onload = function () {
  ...
  function particle() {
    this.speed = {
      x: -1.25 + Math.random() * 3.5,
      y: -0.40 + Math.random() * 2.5
    };

    if (mouse.x && mouse.y) {
      this.location = {
        x: mouse.x + (active * (35 - Math.random() * 58)),
        y: mouse.y + (active * (35 - Math.random() * 58))
      };
    } else {
      this.location = {
        x: W / 2,
  ...
  function draw() {
    ...
    if (mouse.x != undefined) {
      ...
      ctx.fillStyle = "rgba(58,90,190,9)";
    }
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      ctx.strokeStyle = "rgba(58,3,9,.6)";
      ctx.stroke();

      ctx.beginPath();
      ctx.moveTo(p.location.x, p.location.y);
      if ((Math.random() + 0.898) > 0.9) {
        ctx.lineTo(lastX, lastY);
      } else {
